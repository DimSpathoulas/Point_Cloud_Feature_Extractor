...
def decode_bbox_from_heatmap(heatmap, rot_cos, rot_sin, center, center_z, dim,
    ...
    # the 500 possible centers
    # print(feature_map_stride,voxel_size[0],voxel_size[1],point_cloud_range[0],point_cloud_range[1])
    xs = xs * feature_map_stride * voxel_size[0] + point_cloud_range[0]
    ys = ys * feature_map_stride * voxel_size[1] + point_cloud_range[1]

    # print(xs.shape,ys.shape)

    box_part_list = [xs, ys, center_z, dim, angle]  # 7 things
    if vel is not None:
        vel = _transpose_and_gather_feat(vel, inds).view(batch_size, K, 2)  # 9
